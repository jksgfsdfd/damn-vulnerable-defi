...
contract WalletDeployer {
    ...
    function can(address u, address a) public view returns (bool) {
        ...
        bool extcodeCheckError;
        assembly {
            let m := sload(0)
            if iszero(extcodesize(m)) {
                extcodeCheckError := true
            }
        }
        if (extcodeCheckError) {
            console.log("ext code check error");
        }

        bool staticCallError;
        assembly {
            let m := sload(0)
            if iszero(extcodesize(m)) {
                return(0, 0)
            ...
            if iszero(staticcall(gas(), m, p, 0x44, p, 0x20)) {
                staticCallError := true
            }
        }
        if (staticCallError) {
            console.log("static call check error");
        }

        bool returndataError;
